...
class ReadFrameOpencv:
            ...
            for i, tuples in enumerate(frames):
                ...
                if all_in_one:
                    ...
                    for i, frame in enumerate(tuples):
                            ...
                            frame_filename = os.path.join(f"{self.output_folder}/{caminho_arquivo}", f'{tuple_id}_{slice_id}')
    ...
    #     df = pd.DataFrame(columns=["name", "target", "class"])
    #     for i, frame in enumerate(frames):
